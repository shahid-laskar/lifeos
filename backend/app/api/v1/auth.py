...
def register(
    ...
    try:
        ...
        log_event("auth.register.failed", extra={"reason": "email_conflict"})
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Registration could not be completed with the details provided.",
        ) from exc
    log_event("auth.register.success", user_id=user.id)
...
def login(
    ...
    try:
        ...
        log_event("auth.login.failed", extra={"reason": "invalid_credentials"})
        raise HTTPException(
...
def refresh(
    ...
    try:
        decoded = decode_token(body.refresh_token, expected_type=TokenType.REFRESH)
