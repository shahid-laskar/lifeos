...
const COMMON_DISTRACTIONS = ["Phone", "Work/Study", "Family/Kids", "Fatigue", "Rushing", "Worldly Thoughts"];
...
function PrayerJournalPage() {
  ...
  const toggleDistraction = (d: string) => {
    const current = distractions.split(",").map(s => s.trim()).filter(Boolean);
  ...
  const currentDistractionsArr = distractions.split(",").map(s => s.trim()).filter(Boolean);

  const prayerAverages = useMemo(() => {
    if (!journalQuery.data) return [];
    const sums = { fajr: { total: 0, count: 0 }, dhuhr: { total: 0, count: 0 }, asr: { total: 0, count: 0 }, maghrib: { total: 0, count: 0 }, isha: { total: 0, count: 0 } };
  ...
  const topDistractions = useMemo(() => {
    ...
    journalQuery.data.forEach(entry => {
      if (entry.distractions) {
        const items = entry.distractions.split(",").map(s => s.trim()).filter(Boolean);
        items.forEach(item => {
      ...
      <div className="mx-5 mb-6 flex rounded-xl border border-border bg-muted/30 p-1 backdrop-blur-md">
        <button
          ...
          className={cn(
            ...
            activeTab === "journal" ? "bg-card text-foreground shadow-sm scale-[1.02]" : "text-muted-foreground hover:text-foreground"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "flex-1 rounded-lg py-2 text-sm font-medium transition-all duration-300",
            activeTab === "insights" ? "bg-card text-foreground shadow-sm scale-[1.02]" : "text-muted-foreground hover:text-foreground"
          )}
        >
      ...
      <div className="px-5 pb-24">
        {activeTab === "journal" && (
          <div className="space-y-8">
              ...
              <div className="space-y-6 relative z-10">
                ...
                <div className="border-t border-border/50 pt-4">
                  ...
                  >
                    ...
                    {showPrompts ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                  </button>
            ...
            <div>
              <h3 className="mb-4 ml-1 font-semibold text-lg text-foreground/80">Journal History</h3>
                ...
                <div className="space-y-4">
                  {journalQuery.data?.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-10 text-center bg-card/50 rounded-3xl border border-border/50 border-dashed">
                      ...
                      <p className="text-sm font-semibold text-foreground">Your first reflection awaits</p>
                        ...
                        >
                          <div className="flex justify-between items-center mb-3">
                            <div className="flex items-center gap-2">
                              <span className="text-2xl drop-shadow-sm">{iconInfo?.icon || "🌗"}</span>
                              <div>
                                <span className="font-semibold text-foreground capitalize block leading-none">
                                  {PRAYER_LABELS[entry.prayer_name].latin}
                                </span>
                                <span className="text-[11px] text-primary font-medium">{iconInfo?.label}</span>
                              </div>
                            </div>
                            <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded-md">{new Date(entry.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                          </div>
                              ...
                              {entry.distractions && (
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {entry.distractions.split(",").map(d => (
                                    <span key={d} className="text-[10px] bg-destructive/10 text-destructive/80 px-2 py-0.5 rounded-sm font-medium">
                                      {d.trim()}
                                    </span>
                                  ))}
                                </div>
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="rounded-2xl border border-border/50 bg-card p-5 text-center shadow-sm relative overflow-hidden">
                    ...
                    <p className="text-xs font-medium text-muted-foreground mb-1 relative z-10">Weekly Quality</p>
                  ...
                  <div className="rounded-2xl border border-border/50 bg-card p-5 text-center shadow-sm relative overflow-hidden">
                    ...
                    <p className="text-xs font-medium text-muted-foreground mb-1 relative z-10">Monthly Quality</p>
                ...
                <div className="rounded-3xl border border-border/50 bg-card p-6 shadow-sm">
                  ...
                  <div className="h-[200px] w-full -ml-4">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={prayerAverages} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <XAxis 
                          dataKey="name" 
                          axisLine={false} 
                          tickLine={false} 
                        ...
                        <Tooltip 
                          ...
                          contentStyle={{ borderRadius: "12px", border: "1px solid var(--color-border)", boxShadow: "0 4px 12px rgba(0,0,0,0.05)", fontSize: "12px", fontWeight: 600 }}
                          itemStyle={{ color: "var(--color-primary)" }}
                        />
                        <Bar dataKey="avg" radius={[6, 6, 0, 0]} maxBarSize={40}>
                          {prayerAverages.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill="var(--color-primary)" opacity={entry.avg > 0 ? 0.9 : 0.2} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
